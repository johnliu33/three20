...
- (void)layoutSubviews {
  if (_badge || _closeButton) {
    ...
    if (_closeButton) {//johnliu33 20141219
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        _closeButton.origin = CGPointMake(5+imageRect.origin.x - (floor(_closeButton.width*0.05)),
                                    15+imageRect.origin.y - (floor(_closeButton.height*0.05)));

        } else {
         _closeButton.origin = CGPointMake(imageRect.origin.x - (floor(_closeButton.width*0.05)),
                                        10+imageRect.origin.y - (floor(_closeButton.height*0.05)));
        }
    }
  }
}
...
- (TTButton*)closeButton {
  if (!_closeButton && _item.canDelete) {
    ...
    _closeButton.size = CGSizeMake(26,29);//26,29
