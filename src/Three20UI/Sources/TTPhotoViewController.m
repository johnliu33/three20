...
- (void)updateChrome {
  ...
  playButton.enabled = _photoSource.numberOfPhotos_ > 1;
  _previousButton.enabled = _centerPhotoIndex > 0;
  _nextButton.enabled = _centerPhotoIndex >= 0 && _centerPhotoIndex < _photoSource.numberOfPhotos_-1;
}
...
- (void)moveToPhotoAtIndex:(NSInteger)photoIndex withDelay:(BOOL)withDelay {
  ...
  [self moveToPhoto:[_photoSource photoAtIndex_:_centerPhotoIndex]];
...
- (void)updateVisiblePhotoViews {
  ...
  for (NSNumber* key in photoViews.keyEnumerator) {
    ...
    id<TTPhoto> photo = [_photoSource photoAtIndex_:key.intValue];
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)slideshowTimer {
  if (_centerPhotoIndex == _photoSource.numberOfPhotos_-1) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (BOOL)canShowModel {
  return _photoSource.numberOfPhotos_ > 0;
}
...
- (void)moveToNextValidPhoto {
  if (_centerPhotoIndex >= _photoSource.numberOfPhotos_) {
    // We were positioned at an index that is past the end, so move to the last photo
    [self moveToPhotoAtIndex:_photoSource.numberOfPhotos_ - 1 withDelay:NO];
...
- (NSInteger)numberOfPagesInScrollView:(TTScrollView*)scrollView {
  return _photoSource.numberOfPhotos_;
}
...
- (UIView*)scrollView:(TTScrollView*)scrollView pageAtIndex:(NSInteger)pageIndex {
  ...
  id<TTPhoto> photo = [_photoSource photoAtIndex_:pageIndex];
  [self showPhoto:photo inView:photoView];
