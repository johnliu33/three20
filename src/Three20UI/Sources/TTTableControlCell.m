...
///////////////////////////////////////////////////////////////////////////////////////////////////
+ (BOOL)shouldFixControlWidthAndHeight:(UIView*)view {
...
- (void)layoutSubviews {
  ...
  if ([TTTableControlCell shouldSizeControlToFit:_control]) {
    ...
    CGFloat fixControlWidth = self.contentView.width/5;
    if (![TTTableControlCell shouldRespectControlPadding:_control]) {
      contentWidth -= kControlPadding;
    }
    if (self.textLabel.text.length &&
        ![TTTableControlCell shouldFixControlWidthAndHeight:_control]) {
    ...
    if ([TTTableControlCell shouldFixControlWidthAndHeight:_control]) {
      if (IS_IOS_7)
          minX = self.contentView.width - fixControlWidth-kControlPadding+28;
      else
          minX = self.contentView.width - fixControlWidth-kControlPadding;
      contentWidth = fixControlWidth;
      //_control.height -= kControlPadding; //UIButton changed when table scroll johnliu33 201306
    }
