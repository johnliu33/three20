...
- (void)setRootViewController:(UIViewController*)controller {
  if (controller != _rootViewController) {
    ...
    if (nil != _rootContainer) {
      ...
      [self.window.rootViewController = _rootViewController view]; //for ios6 20120920
    }
  }
}
...
- (UIWindow*)window {
  if (nil == _window) {
    ...
    } else {//TTSCreenBounds 20120930 ios6
    CGRect _rect;
      if (TTIsOrienLandscape()) {
            _rect = [UIScreen mainScreen].bounds;

      } else {
            _rect = TTScreenBounds();
      }
      _window = [[[self windowClass] alloc] initWithFrame:_rect];
